...
select max(price) as HighestBookprice from books;

select min(price) as Lowestbookprice from books; 

select avg(price) as Averagebookprice from books;

select count(stock) as totalstock from books;
...
select d.department_name, avg(e.salary) as employeesalary
from departments d
left join employees e
on e.department_id=d.department_id
group by d.department_name;
...
select * from customers_new
where membership_type='Gold' and 
customer_id in(
select customer_id from payments
where customer_id is not null
);
